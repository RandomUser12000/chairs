...
sed -i 's/^[[:space:]]*kernel.randomize_va_space[[:space:]]*=.*/kernel.randomize_va_space =
2/' /etc/sysctl.conf
else
...
sed -i 's/^[[:space:]]*net.ipv4.tcp_syncookies[[:space:]]*=.*/net.ipv4.tcp_syncookies = 1/'
/etc/sysctl.conf
else
...
sed -i 's/APT::Periodic::Update-Package-Lists.*/APT::Periodic::Update-Package-Lists "1";/'
...
sed -i
's|//\s*"\${distro_id}:\${distro_codename}-security";|"\${distro_id}:\${distro_codename}-security";|'
\
